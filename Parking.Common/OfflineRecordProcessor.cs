...
{
    public class OfflineRecordProcessor
    {
        private const string queuerFileName = "OfflineRecords.txt";
        ...
        private static ParkingDatabaseFactory _parkingDatabaseFactory;
        private readonly static object FileLock = new object();

        public static void Queue(object Object, Application application)
        {
            _parkingDatabaseFactory = new ParkingDatabaseFactory(application);
            ...
            lock (FileLock)
            {
                switch (application)
                {
                    ...
                    case Application.TicketDispenserClient:
                        EnqueueTicketDispenserClientRecords(Object);
                        break;
        ...
        public static void Dequeue(Application application)
        {
            _parkingDatabaseFactory = new ParkingDatabaseFactory(application);
            ...
            lock (FileLock)
            {
                switch (application)
                {
                    case Application.ManualPayStation:
                        break;
                    case Application.TicketDispenserClient:
                        DequeueTicketDispenserClientRecords();
                        break;
        ...
        public static void EnqueueTicketDispenserClientRecords(object Object)
        {
            ...
            using (StreamWriter streamWriter = File.AppendText(filePath))
            {
                try
                {
                    ...
                    streamWriter.Write($"{ticket.TicketNumber}(TP){ticket.ValidationNumber}(TP){ticket.QRCode}(TP){ticket.VehicleNumber}(TP){(int)ticket.VehicleType}(TP){ticket.EntryTime}(TP){ Convert.ToBase64String(driverImageBytes)}(TP){ Convert.ToBase64String(vehicleImageBytes)}(TICKET){Environment.NewLine}");
                    streamWriter.Close();
        ...
        public static void DequeueTicketDispenserClientRecords()
        {
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                var configurationSettings = (TickerDispenserClientSettings)ConfigurationReader.GetConfigurationSettings(Application.TicketDispenserClient);
                ...
                foreach (var ticket in ticketList)
                {
                    try
                    {
                        _parkingDatabaseFactory.SaveVehicleEntry(configurationSettings.DeviceId, ticket);
                    }
                    catch (Exception)
                    {
                        //Queue all tickets again in case of network Failure
                        EnqueueTicketDispenserClientRecords(ticket);
                    }
                }
            }
        }
